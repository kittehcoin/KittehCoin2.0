...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing `BitcoinTestFramework` (upstream class name for the kittehcoind harness), place overrides for the
...
with the kittehcoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a kittehcoind.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the KittehCoin2.0 Core node application logic. For custom behaviour, subclass the
...
Test objects for interacting with a kittehcoind node over the p2p interface.
