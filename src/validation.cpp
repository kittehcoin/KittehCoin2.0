    ...
    for (const CTxInput& txin : tx.GetInputs())
    {
        ...
        if (ptxConflicting) {
            if (!setConflicts.count(ptxConflicting->GetHash()))
            {
                ...
                // KittehCoin: Only support BIP125 RBF when -mempoolreplacement arg is set
...
namespace {
/**
 * Portable consensus RNG: SHA256d(prevHash || height || domain) → uniform in [1, range].
 * Avoids std::uniform_int_distribution (implementation-defined across STL vendors).
 */
uint32_t DeterministicUniform(const uint256& prevHash, int nHeight, uint32_t range)
{
    assert(range >= 1);
    CHashWriter ss(SER_GETHASH, PROTOCOL_VERSION);
    ss << prevHash;
    ss << nHeight;
    ss << std::string("KittehCoin2/Subsidy");
    const uint256 h = ss.GetHash();
    uint64_t v = 0;
    memcpy(&v, h.begin(), sizeof(v));
    return 1u + static_cast<uint32_t>(v % range);
}

CAmount ScheduledSubsidy(int nHeight, const uint256& prevHash)
{
    // Genesis coinbase is fixed in chainparams (CreateGenesisBlock); keep subsidy deterministic.
    if (nHeight == 0) {
        return 1000 * COIN;
    }

    // Guaranteed minimum 1,000 MEOWS; max halves by tier; flat 2,000 after block 700,000.
    CAmount nSubsidy = 1000 * COIN;

    const int height1 = 200000;
    const int height2 = 400000;
    const int height3 = 500000;
    const int height4 = 600000;
    const int height5 = 700000;

    // Same shape as original KittehCoin: rand in [1, R] then (1+rand) → [2, R+1], floored at 1000.
    if (nHeight < height1) {
        const uint32_t rand = DeterministicUniform(prevHash, nHeight, 49999);
        const CAmount candidate = (1 + static_cast<CAmount>(rand)) * COIN;
        if (candidate > nSubsidy) nSubsidy = candidate;
    } else if (nHeight < height2) {
        const uint32_t rand = DeterministicUniform(prevHash, nHeight, 24999);
        const CAmount candidate = (1 + static_cast<CAmount>(rand)) * COIN;
        if (candidate > nSubsidy) nSubsidy = candidate;
    } else if (nHeight < height3) {
        const uint32_t rand = DeterministicUniform(prevHash, nHeight, 12499);
        const CAmount candidate = (1 + static_cast<CAmount>(rand)) * COIN;
        if (candidate > nSubsidy) nSubsidy = candidate;
    } else if (nHeight < height4) {
        const uint32_t rand = DeterministicUniform(prevHash, nHeight, 6249);
        const CAmount candidate = (1 + static_cast<CAmount>(rand)) * COIN;
        if (candidate > nSubsidy) nSubsidy = candidate;
    } else if (nHeight < height5) {
        const uint32_t rand = DeterministicUniform(prevHash, nHeight, 3124);
        const CAmount candidate = (1 + static_cast<CAmount>(rand)) * COIN;
        if (candidate > nSubsidy) nSubsidy = candidate;
    } else {
        nSubsidy = 2000 * COIN;
...
{
    return GetBlockSubsidy(nHeight, uint256(), 0, consensusParams);
}

CAmount GetBlockSubsidy(int nHeight, const uint256& prevHash, const Consensus::Params& consensusParams)
{
    return GetBlockSubsidy(nHeight, prevHash, 0, consensusParams);
}

CAmount GetBlockSubsidy(int nHeight, const uint256& prevHash, CAmount nMoneySupplyPrev, const Consensus::Params& consensusParams)
{
    (void)consensusParams;
    CAmount nSubsidy = ScheduledSubsidy(nHeight, prevHash);

    // Hard cap circulating supply at MAX_MONEY (25B MEOWS). Fees are not new issuance.
    if (nMoneySupplyPrev >= MAX_MONEY) {
        return 0;
    }
    const CAmount remaining = MAX_MONEY - nMoneySupplyPrev;
    if (nSubsidy > remaining) {
        nSubsidy = remaining;
    }
    return nSubsidy;
}
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Special case for the genesis block: skip full BIP30/script path (no pprev), but still
    // apply the coinbase to the UTXO set so the launch mint is spendable after maturity.
    // Also seed nMoneySupply so the MAX_MONEY hard cap accounts for genesis.
    if (block.GetHash() == chainparams.GetConsensus().hashGenesisBlock) {
        CAmount genesisMint = 0;
        for (const auto& out : block.vtx[0]->vout) {
            genesisMint += out.nValue;
            if (!MoneyRange(genesisMint)) {
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-genesis-amount");
            }
        }
        pindex->nMoneySupply = genesisMint;
        if (!fJustCheck) {
            CTxUndo undoDummy;
    ...
    uint256 prevHash;
    CAmount nMoneySupplyPrev = 0;
    if (pindex->pprev) {
        prevHash = pindex->pprev->GetBlockHash();
        nMoneySupplyPrev = pindex->pprev->nMoneySupply;
    }
    const CAmount nBlockSubsidy = GetBlockSubsidy(pindex->nHeight, prevHash, nMoneySupplyPrev, chainparams.GetConsensus());
    CAmount blockReward = nFees + nBlockSubsidy;
    if (block.vtx[0]->GetValueOut() > blockReward) {
        LogPrintf("ERROR: ConnectBlock(): coinbase pays too much (actual=%d vs limit=%d)\n", block.vtx[0]->GetValueOut(), blockReward);
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-amount");
    }
    // Track actual minted supply (coinbase − fees), not the scheduled ceiling. Underpaying
    // miners therefore do not "burn" future issuance room against the MAX_MONEY hard cap.
    CAmount nMinted = block.vtx[0]->GetValueOut() - nFees;
    if (nMinted < 0) {
        LogPrintf("ERROR: ConnectBlock(): coinbase below fees (actual=%d fees=%d)\n", block.vtx[0]->GetValueOut(), nFees);
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-amount");
    }
    if (nMinted > nBlockSubsidy) {
        nMinted = nBlockSubsidy; // defensive; ruled out by the check above
    }
    if (nMoneySupplyPrev > MAX_MONEY - nMinted) {
        LogPrintf("ERROR: ConnectBlock(): money supply overflow\n");
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-amount");
    }
    pindex->nMoneySupply = nMoneySupplyPrev + nMinted;
