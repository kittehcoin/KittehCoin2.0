...
const char * const BITCOIN_CONF_FILENAME = "kittehcoin.conf";
...
#else
    const char* pszModule = "kittehcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KittehCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/KittehCoin";
#else
    // Unix-like
    return pathRet / ".kittehcoin";
#endif
#endif
}
