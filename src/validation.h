...
/** KittehCoin 2.0 subsidy — portable hash RNG + MAX_MONEY circulating supply cap. */
...
/** Convenience: assume zero prior supply (tests / display without tip context). */
CAmount GetBlockSubsidy(int nHeight, const uint256& prevHash, const Consensus::Params& consensusParams);
/** Convenience wrapper using a null previous hash and zero prior supply. */
