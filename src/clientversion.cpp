...
// Copyright (c) 2013-2026 The KittehCoin2.0 Core developers
 ...
 * for both kittehcoind and kittehcoin-qt, to make it harder for attackers to
 * target servers or GUI users specifically.
 */
const std::string CLIENT_NAME("KittehCoin2.0");
...
/** Build identifier used in logs / dumps — matches the friendly release version. */
const std::string CLIENT_BUILD(
    CLIENT_VERSION_BUILD == 0
        ? (STRINGIZE(CLIENT_VERSION_MAJOR) "." STRINGIZE(CLIENT_VERSION_MINOR) "." STRINGIZE(CLIENT_VERSION_REVISION))
        : (STRINGIZE(CLIENT_VERSION_MAJOR) "." STRINGIZE(CLIENT_VERSION_MINOR) "." STRINGIZE(CLIENT_VERSION_REVISION) "." STRINGIZE(CLIENT_VERSION_BUILD))
);
...
/**
 * User-facing version string shown in About / -version.
 * Clean release form only — no git hash or "-dirty" suffix.
 */
