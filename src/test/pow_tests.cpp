...
void sanity_check_chainparams(const ArgsManager& args, std::string chainName)
{
    ...
    /* KittehCoin: we allow overflowing by 1 bit
...
{
    // Help-text stub may construct params; selecting signet must fail
    BOOST_CHECK_THROW(SelectParams(CBaseChainParams::SIGNET), std::runtime_error);
}

BOOST_AUTO_TEST_CASE(kgw_min_diff_window_short)
{
    const auto chainParams = CreateChainParams(*m_node.args, CBaseChainParams::MAIN);
    const Consensus::Params& params = chainParams->GetConsensus();
    BOOST_CHECK(params.fPowKimotoGravityWell);
    BOOST_CHECK_EQUAL(params.nKGWPastBlocksMin, 4U);

    // While tip height < PastBlocksMin, next work is powLimit
    CBlockIndex genesis;
    genesis.nHeight = 0;
    genesis.nTime = 1786896000;
    genesis.nBits = 0x1e0ffff0;
    CBlockHeader hdr;
    hdr.nTime = genesis.nTime + 60;
    const unsigned int limit = UintToArith256(params.powLimit).GetCompact();
    BOOST_CHECK_EQUAL(GetNextWorkRequired(&genesis, &hdr, params), limit);

    // After PastBlocksMin blocks of history, KGW should return a compact target ≤ powLimit
    CBlockIndex blocks[6];
    blocks[0] = genesis;
    for (int i = 1; i <= 5; ++i) {
        blocks[i].pprev = &blocks[i - 1];
        blocks[i].nHeight = i;
        blocks[i].nTime = genesis.nTime + i * 60;
        blocks[i].nBits = 0x1e0ffff0;
    }
    hdr.nTime = blocks[5].nTime + 60;
    const unsigned int next = GetNextWorkRequired(&blocks[5], &hdr, params);
    arith_uint256 bn;
    bool neg, over;
    bn.SetCompact(next, &neg, &over);
    BOOST_CHECK(!neg && !over && bn != 0);
    BOOST_CHECK(bn <= UintToArith256(params.powLimit));
}
