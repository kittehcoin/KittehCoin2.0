...
The KittehCoin2.0 Core daemon can be configured to act as a trusted "border
router", implementing the kittehcoin wire protocol and relay, making
...
The ZeroMQ feature in KittehCoin2.0 Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building kittehcoind:
...
For instance:
    ...
    $ kittehcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/kittehcoind.tx.raw \
...
These options can also be provided in kittehcoin.conf.
...
From the perspective of kittehcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into kittehcoind directly. Furthermore, no information is
...
using. KittehCoind appends an up-counting sequence number to each
